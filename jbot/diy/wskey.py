...
async def myaddwskey(event):
    try:
        ...
        if len(text) > 1:
            ...
            elif os.path.exists("/ql/data/scripts/ql_pandaAPI_refreshCK.py") and not os.path.exists("/ql/data/db/wskey.list"):
                text += "\n将自动更新cookie列表，自行查看更新情况"
                await cmd("task /ql/data/scripts/ql_pandaAPI_refreshCK.py")
            elif os.path.exists("/ql/data/raw/ql_pandaAPI_refreshCK.py") and not os.path.exists("/ql/data/db/wskey.list"):
                text += "\n将自动更新cookie列表，自行查看更新情况"
            elif os.path.exists("/ql/data/scripts/ql_pandaAPI_refreshCK.py") and os.path.exists("/ql/data/db/wskey.list"):
                text += "\n由于使用wskey.list存储，无法执行scripts目录下的ql_pandaAPI_refreshCK.py脚本"
            elif os.path.exists("/ql/data/raw/ql_pandaAPI_refreshCK.py") and os.path.exists("/ql/data/db/wskey.list"):
